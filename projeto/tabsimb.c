 ...
 *
 * tabsimb.c
 * Implementação da tabela de símbolos (ATUALIZADO PARA EXIBIR PARÂMETROS)
 *******************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void ts_exit_scope(void) {
    ...
    /* Ao sair do escopo, o next_address deveria teoricamente voltar? 
       Para MEPA simples com endereço sequencial global de var, não necessariamente, 
       mas para funções locais sim. Mantemos lógica original do aluno. */
    // next_address = scope_stack->alloc_base; // Opcional, dependendo da alocação MEPA
    free(tmpS);
}
...
RegistroTS* ts_inserir(char *lexema, Categoria cat, TipoAtomo tipo, int endereco) {
    ...
    r->arr_size = 0;
    r->param_count = 0;
    /* Inicializa array de tipos com 0 */
    memset(r->param_types, 0, sizeof(r->param_types));
...
void ts_write_file(const char *filename) {
    ...
    for (int d = i-1; d >= 0; d--) {
        ...
        /* Inverter ordem para impressão (opcional, mas comum) */
        ...
        for (int k=n-1; k>=0; k--) {
            ...
            switch(s->tipo) {
                ...
                case sSTRING: fprintf(f, "string"); break; 
                default: fprintf(f, "unknown"); break;
            }
            
            if (s->arr_size > 0) fprintf(f, "[%d]", s->arr_size);
            
            /* Melhoria: Imprimir assinatura da função */
            if (s->cat == CAT_FUNC) {
                fprintf(f, " params=%d (", s->param_count);
                for (int p=0; p < s->param_count; p++) {
                    switch(s->param_types[p]) {
                        case sINT: fprintf(f, "int"); break;
                        case sFLOAT: fprintf(f, "float"); break;
                        case sCHAR: fprintf(f, "char"); break;
                        default: fprintf(f, "?"); break;
                    }
                    if (p < s->param_count - 1) fprintf(f, ", ");
                }
                fprintf(f, ")");
            }
            fprintf(f, "\n");
